...
public abstract class VideoStream extends MediaStream {
	...
	protected int mVideoEncoder, mCameraId = 0; // Camera.CameraInfo.CAMERA_FACING_BACK
	...
	private boolean prepareRecorder(boolean check) {
		unlockCamera();
	...
	protected synchronized void createCamera() throws RuntimeException, IOException {
		...
		if (mCamera == null) {
			...
			try {
				...
				destroyCamera();
				Log.d(TAG, "RUNTIME EXC, CAN'T SET MCAMERA PARAMETERS");
				throw e;
			} catch (IOException e) {
				destroyCamera();
				throw e;
	...
	protected synchronized void destroyCamera() {
		if (mCamera != null) {
			...
			try {
				...
				Log.e(TAG,e.getMessage()!=null?e.getMessage():"unknown error camera release");
			}
			mCamera = null;
			mUnlocked = false;
			mPreviewStarted = false;
	...
	protected void unlockCamera() {
		if (!mUnlocked) {
			...
			try {
				mCamera.unlock();
                Log.d(TAG, "Unlocking camera succesfull");
			} catch (Exception e) {
				Log.e(TAG,e.getMessage());
			}
			mUnlocked = true;
